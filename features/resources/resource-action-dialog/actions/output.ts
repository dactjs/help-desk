...
export const output: FormAction = async (_, formData) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!ability.can("output", subject("Resource", resource)))
        throw new Error(errors.FORBIDDEN_ERROR);

      await tx.resource.update({
        where: { id: result.data.resource },
        data: {
          status: ResourceStatus.DISCARDED,
          assignedToId: null,
          traces: {
            create: {
              type: ResourceTraceType.OUTPUT,
